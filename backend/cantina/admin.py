    ...
    if route_id is not None:
        ...
        if request.method == "POST":
            ...
            if name is not None and name != route.name:
                flash(
                    f"Nome da rota alterada com sucesso ({route.name} -> {name})!",
                    category="success",
                )
                route.name = name
            db.session.commit()
        context = {"route": route, "is_editor": is_editor}
    else:
        context = {"routes": Route.query.all()}
    ...
    if category_page_id is not None:
        ...
        if request.method == "POST":
            ...
            if name is not None and name != category_page.name:
                flash(
                    f"Nome da categoria de página alterada com sucesso ({category_page.name} -> {name})!",
                    category="success",
                )
            ...
            db.session.commit()
        context = {"category_page": category_page, "is_editor": is_editor}
    else:
        context = {"category_pages": CategoryPage.query.all()}
    ...
    if page_id is not None:
        ...
        if request.method == "POST":
            ...
            if title is not None and title != page.title:
                flash(
                    f"Nome da página alterada com sucesso ({page.title} -> {title})!",
                    category="success",
                )
                page.title = title
            if description is not None and description != page.description:
                flash(
                    f"Descrição da página alterada com sucesso ({page.description} -> {description})!",
                    category="success",
                )
            ...
            if category_page_id is not None and category_page_id != str(
                ...
                if category_page is None:
                    ...
                    flash(
                        f"Categoria de página da página alterada com sucesso ({page.category_page_id} -> {category_page_id})!",
                        category="success",
                    )
            ...
            if appear_navbar != page.appear_navbar:
                flash(
                    f"Aparece na Barra de Navegação da página alterada com sucesso ({page.appear_navbar} -> {appear_navbar})!",
                    category="success",
                )
    ...
    if action == "view":
        ...
        if request.method == "POST":
            ...
            if name is not None and name != role.name:
                flash(
                    f"Nome do cargo alterado com sucesso ({role.name} -> {name})!",
                    category="success",
                )
                role.name = name
            description = request.form.get("description")
            if description is not None and description != role.description:
                flash(
                    f"Descrição do cargo alterado com sucesso ({role.description} -> {description})!",
                    category="success",
                )
                ...
                try:
                    ...
                    for route_id in old_allowed_routes:
                        if route_id not in allowed_routes:
                            flash(
                                f"Acesso à Rota '{routes[route_id].name}' (#{route_id}) foi removida do cargo!",
                                category="success",
                            )
                    for route_id in allowed_routes:
                        if route_id not in old_allowed_routes:
                            flash(
                                f"Acesso à Rota '{routes[route_id].name}' (#{route_id}) foi adicionada ao cargo!",
                                category="success",
                            )
        ...
        role = role.as_dict()
        role["allowed_routes"] = json.loads(role["allowed_routes"])
        context = {"role": role, "routes": routes}
    elif action == "add":
        if request.method == "POST":
            name = request.form.get("name")
            description = request.form.get("description")
        ...
        flash(
            "Por enquanto essa funcionalidade está desativada para demais testes!",
            category="warning",
        )
        return redirect(url_for("roles", action="view"))
        # role_id = request.args.get('role_id')
...
def checkout_payroll():
    ...
    users_balance_payroll_gt0 = User.query.filter(
        User.balance_payroll > 0, User.name.ilike(f"%{search_query}%")
    ).all()
    if action == "view":
        ...
        if value <= 0:
            flash(
                "Então, espertinho(a), por favor, insira um estritamente positivo!",
                category="error",
            )
            return redirect(url_for("checkout_payroll", action="view"))
        if value > float(target_user.balance_payroll):
            flash(
                "Por favor, insira um valor menor ou igual ao saldo devedor do usuário!",
                category="error",
            )
            return redirect(url_for("checkout_payroll", action="view"))
        ...
        db.session.commit()
        flash(
            f"O saldo devedor do usuário {target_user.name} ({target_user.id}) foi atualizado de R$ {old_balance} para R$ {target_user.balance_payroll}!",
            category="success",
        )
        return redirect(url_for("checkout_payroll", action="view"))
    else:
